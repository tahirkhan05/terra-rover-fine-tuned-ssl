    ...
    def generate_response(self, query_type, image_key, **kwargs):
        try:
            ...
            try:
                ...
                # Handle new Bedrock Converse API response format
                if 'output' in response_json:
                    # New API format: {"output": {"message": {"content": [{"text": "..."}]}}}
                    message = response_json.get('output', {}).get('message', {})
                    content = message.get('content', [])
                    if content and isinstance(content, list):
                        return content[0].get('text', "No agricultural analysis generated.")
                    ...
                    return "Unable to parse agricultural analysis response."
